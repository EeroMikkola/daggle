	...
	daggle_node_declare_fn declare)
{
	...
	RETURN_IF_ERROR(resource_container_register_node(resource_container,
		node_type, declare));
...
daggle_error_code_t
daggle_plugin_register_type(daggle_instance_h instance, const char* type_name,
	...
	daggle_data_deserialize_fn deserializer)
{
	...
	RETURN_IF_ERROR(resource_container_register_type(resource_container,
		type_name, cloner, freer, serializer, deserializer));
