	...
	daggle_task_h task, void* context)
{
	...
	daggle_node_get_port_by_name(
		math_context->node, "operation", &operationPort);
	...
	daggle_task_h task, void* context)
{
	...
	daggle_task_create(
		math_read_fn, NULL, math_context, "math_read\0", &reader_task);

	daggle_task_h calculator_task;
	daggle_task_create(math_calculate_fn,
		NULL,
		math_context,
		"math_calculate\0",
		&calculator_task);
	...
	daggle_task_create(math_write_fn,
		math_write_dispose,
		math_context,
		"math_write\0",
		&writer_task);
...
DEFAULT_VALUE_GENERATOR(
	math_gdv_first, int32_t, 5, INT_TYPE)
DEFAULT_VALUE_GENERATOR(
	math_gdv_second, int32_t, 2, INT_TYPE)
DEFAULT_VALUE_GENERATOR(
	math_gdv_operation, int32_t, 0, INT_TYPE)

void
math(
	daggle_node_h handle)
{
	daggle_node_declare_input(handle, "first", DAGGLE_INPUT_IMMUTABLE_REFERENCE, math_gdv_first);
	daggle_node_declare_input(handle, "second", DAGGLE_INPUT_IMMUTABLE_REFERENCE, math_gdv_second);
